...
switch ($_GET["op"]){
	case 'guardaryeditar':
		if (empty($idventa)){
			$rspta=$venta->insertar($idcliente,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$fecha_hora,$total_venta,$_POST["idarticulo"],$_POST["cantidad"],$_POST["precio_venta"],$_POST["impuesto"]);
		...
		while ($reg = $rspta->fetch_object())
				{
					echo 	'<tr class="filas"><td></td>
								...
								<td>'.(((($reg->precio_venta*$reg->impuesto)/100)+$reg->precio_venta)*$reg->cantidad).'</td>
							</tr>';
					$total=$total+(((($reg->precio_venta*$reg->impuesto)/100)+$reg->precio_venta)*$reg->cantidad);
				}
		echo '<tfoot>
                                    <th></th>
                                    <th></th>
	...
	case 'listarArticulosVenta':
		...
		$rspta=$articulo->listarActivosVenta();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
				"0"=>'<button class="btn btn-warning" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\','.$reg->venta.')"><span class="fa fa-plus"></span></button>',
 				"1"=>$reg->nombre,
 				"2"=>$reg->categoria,
 				"3"=>$reg->codigo,
 				"4"=>$reg->stock,
